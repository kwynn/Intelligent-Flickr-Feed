...
API_KEY = '279885b2f6cde5c5b6cb47ec54287b71'
API_SECRET = '136ad81f7b928c64'
email = None
password = None
AUTH = False
debug = False
        ...
        if hasattr(photo.tags, "tag"):
            if isinstance(photo.tags.tag, list):
                self.__tags = [tag.text for tag in photo.tags.tag]
                              #[Tag(tag.id, User(tag.author), tag.raw, tag.text) \
                              #for tag in photo.tags.tag]
            else:
                tag = photo.tags.tag
                self.__tags = [tag.text] #[Tag(tag.id, User(tag.author), tag.raw, tag.text)]
        else:
            self.__tags = []
